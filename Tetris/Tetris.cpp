...
void setMap(bool map[][WIDTH]) {		// 맵 초기화(테두리 생성).
...
void printMap(const bool map[][WIDTH]) {		// 초기화된 맵 출력
	gotoxy(0, 0);
...
void removeBlock(const bool map[][WIDTH], BLOCK* pBlock) {		// 출력된 블럭의 좌표에 공백을 덮어씌워 지운다.
...
void rotateBlockPoint(const bool map[][WIDTH], BLOCK* pBlock) { // 블럭을 회전시키는 함수.
	POINT* point = pBlock->blockPoint;
...
void putBlock(const bool map[][WIDTH], BLOCK* pBlock) { // 저장된 좌표로 이동하여 블럭을 출력함.
...
void putBlockPrev(const bool map[][WIDTH], BLOCK* pBlock) {		// 드랍 중인 블록의 미리보기 출력.
	for (int i = 0; i < BLOCK_SIZE; i++) {
...
void removeBlockPrev(const bool map[][WIDTH], BLOCK* pBlock) {		// 미리보기 위에 공백을 출력하여 덮어씌움.
	for (int i = 0; i < BLOCK_SIZE; i++) {
		gotoxy(2 * pBlock->blockPoint[i].x, pBlock->blockPoint[i].y + pBlock->deltaY);
		printf("  ");
	}
...
int getDeltaY(const bool map[][WIDTH], BLOCK* pBlock) {	// 떨어지는 블럭과 바닥 간의 거리를 리턴하는 함수.
